...
chromosomes <- unique(mbias_data$chr)

# Loop through each chromosome and plot
for (chr_name in chromosomes) {
  chr_data <- mbias_data %>% filter(chr == chr_name)
  
  p <- ggplot(chr_data, aes(x = Position, y = methPerc, color = context, linetype = strand)) +
    geom_line() +
    labs(
      title = paste("Mbias Plot for", chr_name),
      x = "Read Position",
      y = "Methylation Percentage"
    ) +
    theme_minimal()
  ...
  # Save plot
  ggsave(filename = paste0("mbias_plot_", chr_name, ".png"), plot = p, width = 10, height = 6)
}
```
